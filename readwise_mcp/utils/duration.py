...
def parse_duration(duration_str: str) -> Tuple[date, date]:
    """
    Parses a duration string (e.g., '1w', '2h', '30m') and returns a datetime range.
    ...
    Args:
        duration_str: The duration string. Valid units: 'h' (hour), 'm' (minute), 'w' (week).
    ...
    match = re.fullmatch(r"(\d+)([hwm])", duration_str)
    if not match:
        raise ValueError(f"Invalid duration format: '{duration_str}'. Expected format like '1w', '2h', '30m'.")
    ...
    if unit == "h":
        ...
        raise ValueError(f"Unknown duration unit: '{unit}'. Valid units are 'h', 'm', 'w'.")
